...
# --- 3. SIDEBAR (STEUERUNG) ---
# WICHTIG: Nur ein Sidebar-Block um DuplicateElementId zu vermeiden!
with st.sidebar:
    st.header("⚙️ Steuerung")
    sel_region = st.selectbox("Region wählen", ["Deutschland", "Brandenburg/Berlin", "Europa"])
    ...
    st.markdown("---")
    st.caption("Daten: DWD ICON-D2 | Panzer-Modus")

# --- 4. DATA LOADER ---
@st.cache_data(ttl=900)
def fetch_safe(p_key, hr):
    for off in [2, 3, 4, 6]:
        ...
        try:
            ...
            if r.status_code == 200:
                with open(f"{p_key}.bz2", "wb") as f: f.write(r.content)
...
with st.spinner('🛰️ Lade Profi-Daten...'):
    data, lons, lats, run_info = fetch_safe(p_map[sel_param], sel_hour)
...
if data is not None:
    ...
    if sel_param == "Temperatur 2m (°C)":
        ...
        # --- NEU: TEMPERATUR-ZAHLEN ---
        # Wir nehmen nur jeden x-ten Punkt, damit die Karte lesbar bleibt
        skip = 25 if sel_region == "Deutschland" else 8
        for i in range(0, lats.shape[0], skip):
            for j in range(0, lons.shape[1], skip):
                lat, lon, val = lats[i, j], lons[i, j], temp_c[i, j]
                # Nur Zahlen im sichtbaren Bereich zeichnen
    ...
    st.error("DWD-Server antwortet nicht. Bitte kurz warten und Regler bewegen.")
