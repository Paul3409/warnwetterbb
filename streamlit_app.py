...
# --- KONFIGURATION ---
st.set_page_config(page_title="WarnwetterBB | Profi-Check", layout="wide")

# --- FARBSKALEN (KONTINUIERLICH FÜR FLÜSSIGE ÜBERGÄNGE) ---

# 1. Signifikantes Wetter (Bleibt diskret)
WW_LEGEND_DATA = {
...
# 2. Temperatur (Deine Skala - jetzt flüssig interpoliert)
    ...
    for off in [2, 3, 4, 6]:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                return xr.open_dataset(f"{p_key}.grib", engine='cfgrib'), dt_s
        except: continue
    return None, None

# --- PLOTTING ---
p_map = {"Signifikantes Wetter": "ww", "Temperatur 2m (°C)": "t_2m", "Windböen (km/h)": "vmax_10m"}
with st.spinner('Berechne hochauflösende Karte...'):
    ds, run_info = fetch_now(p_map[sel_param], sel_hour)

if ds:
    # Dimensionen radikal glätten (Verhindert Dimensions-Error)
    ...
    if sel_param == "Temperatur 2m (°C)":
        ...
        # FIX: Legende ohne zorder initialisieren, dann separat setzen
    ...
    st.warning("🔄 Suche nach aktuellen DWD-Daten...")
